...
AF_Stepper myStepperY(100,1);            
AF_Stepper myStepperX(100,2);  
...
float StepsPerMillimeterX = 150.0;
  ...
  while( currentIndex < charNB ) {
    ...
    case 'G':
      ...
      case 0:                                   // G00 & G01 - Movement or fast movement. Same here
      {
        ...
        switch ( atoi( buffer ) ){
          case 300:{
            //PenUp down in the new gcode version 
            char* indexS = strchr( line+currentIndex, 'Z' );
...
void resetHead() {
  ...
  int xbuttonState = 1;
  xbuttonState = digitalRead(xbutton);
  if(xbuttonState == LOW){
    Serial.print("Resetting X : ");
    while(xbuttonState == LOW){
      myStepperX.onestep(-1,DOUBLE);
      delay(2);
      xbuttonState = digitalRead(xbutton);
    }
  }
  ...
  if(ybuttonState == LOW){
    Serial.print("Resetting Y : ");
    while(ybuttonState == LOW){
      myStepperY.onestep(1,DOUBLE);
      delay(2);
      ybuttonState = digitalRead(ybutton);
    }
  }
